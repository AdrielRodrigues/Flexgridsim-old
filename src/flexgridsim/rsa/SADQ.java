 ...
 * @author Rafael
 */
public class SADQ implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		if (id >= 0) {
			...
			if(flow.getCOS() == 2) {
				pathBack = getbestPathBackR(graph, flow.getSource(), flow.getDestination(), demandInSlots, path, true);
			} else {
			...
			*/
			...
			if(idc >= 0) {
				...
				st.setErroLightPath("Proteção", idc);
			}
		...
		if(id<0){
			cp.blockFlow(flow.getID());
			st.setErroLightPath("Primário", id);
			return;
		}
