 ...
 * @author Helder
 */
public class xtf implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		long id=-1;
		int guardBand=1; //Maybe change the slots number
		int demandInSlots = (int) (Math.ceil(flow.getRate() / (double) pt.getSlotCapacity()))+ guardBand;
		ArrayList<int[]> paths = new ArrayList<int[]>();
		Path path;
		
		//Prioridade de núcleos com cálculo do xtps
		
		paths = getKShortestPath(graph, flow.getSource(), flow.getDestination(), demandInSlots,false);
		...
		path = allocate(paths, demandInSlots);
 ...
 */
	...
	public ArrayList<int[]> getKShortestPath(WeightedGraph G,int src, int dst, int demand, boolean overlap){
		ArrayList<int[]> paths = new ArrayList<int[]>();
		KShortestPaths kShortestPaths = new KShortestPaths();
		int[][] kPaths = kShortestPaths.dijkstraKShortestPaths(G, src, dst, 2);
		if(kPaths==null)
			return null;
		int []links;
		//ArrayList<Slot> channel = new ArrayList<Slot>();
		...
		for(int i = 0; i < pt.getNumSlots(); i++) {
			if(spectrum[i]) {
				...
				if(counter == demandInSlots) {
					...
					if(counter > demandInSlots && firstP == -1)
						firstP = first;
					counter = 0;
				}
			}
	...
	public Path allocate(ArrayList<int[]> paths, int demandInSlots) {
		int first = -1;
		for(int[] path:paths) {
			int[] core = {0, 1, 2, 3, 4, 5, 6};
			for(int i:core) {
				first = findExactFit(demandInSlots, i, path);
				
				
				if(first == -1 || first+demandInSlots<=320)
					continue;
				else {
					ArrayList<Slot> channel = new ArrayList<Slot>();
					for(int link:path) {
						for(int s = first; s < first + demandInSlots; s++) {
							channel.add(new Slot(i, s, link));
						}
					}
					return new Path(path, channel);
				}
			}
		}
		return null;
