 ...
 * @author Helder
 */
public class paper implements RSA {
 ...
 */
	...
	public ArrayList<Slot> allocate(int[] path, int demandInSlots) {
		...
		for(int core = 0; core < 7; core++) {
			...
			for(int link:path) {
				...
				for(int i = 0; i < 320; i++) {
					if(!sp[core][i]) {
						if(core==0) {
							if(!sp[1][i] || !sp[5][i] || !sp[6][i]) {
								aoc++;
							}
						} else if(core==5) {
							if(!sp[0][i] || !sp[4][i] || !sp[6][i]) {
								aoc++;
							}
						} else if(core==6) {
							if(!sp[0][i] || !sp[1][i] || !sp[2][i] || !sp[3][i] || !sp[4][i] || !sp[5][i]) {
								aoc++;
							}
						} else {
							if(!sp[core+1][i] || !sp[core-1][i] || !sp[6][i]) {
								aoc++;
							}
						}
					}
		...
		int[] cores_sort = ordenar(aocs);
		
		for(int core:cores_sort) {
			first = findExactFit(demandInSlots, core, path);
			...
			if(first == -1 || first+demandInSlots>320)
				...
				for(int link:path) {
					for(int s = first; s < first + demandInSlots; s++) {
						channel.add(new Slot(core, s, link));
					}
				}
				return channel;
			}
