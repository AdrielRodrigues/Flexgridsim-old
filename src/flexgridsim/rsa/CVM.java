...
/**
 * The Class MINIMIZE FRAGMENTATION 1.
 * 
 * @author Adriel
 * 
 * 
 */
public class CVM implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		do{
			...
			if(path!=null)
				if (modulation == Modulations.getModulationByDistance(getPhysicalDistance(path.getLinks())))
					find = true;
				else
					modulation--;
		...
		if (path == null) {
			...
			return;
		} else if (path.getLinks() == null || path.getSlotList().isEmpty() || !find) {
			cp.blockFlow(flow.getID());
			return;
		}
...
//	Diz se dois vetores são disjuntos
	...
	public int fragCore (int[] links, int core) {
		int count = 0;
		for(int l:links) {
			boolean[] c = pt.getLink(l).getSpectrum(core);
			for (boolean s:c) {
				if(!s)
					count++;
			}
