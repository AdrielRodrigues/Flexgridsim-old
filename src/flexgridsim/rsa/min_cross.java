 ...
 * @author Helder
 */
public class min_cross implements RSA {
	...
	public ArrayList<Slot> allocate(int[] path, int demandInSlots) {
		...
		for(int i = 0; i < cores_sort.length; i++) {
			if(i == 6) {
				...
				if(fragmentation_metric(demandInSlots, cores_sort[i], path) <= fragmentation_metric(demandInSlots, cores_sort[i+1], path)) {
