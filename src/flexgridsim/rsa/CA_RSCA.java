 ...
 *
 */
public class CA_RSCA implements RSA {
	...
	public void flowArrival(Flow flow) {
			...
			if (vt.getLightpath(a) != null)
				...
				if (vt.getLightpath(a).getProtection()) {
					...
					if (disjoint(cp.getLightPathsProtectedByLightPath(vt.getLightpath(a)), flow.getLinks())) {// disjunto
						...
						// Diferente
						if ((pt.getLink(vt.getLightpath(a).getLinks()[0]).getSource() == flow.getSource()
								&& pt.getLink(vt.getLightpath(a).getLinks()[vt.getLightpath(a).getLinks().length - 1])
										.getDestination() == flow.getDestination())
								|| (pt.getLink(vt.getLightpath(a).getLinks()[0]).getSource() == flow.getDestination()
										&& pt.getLink(
												vt.getLightpath(a).getLinks()[vt.getLightpath(a).getLinks().length - 1])
												.getDestination() == flow.getSource())) {
		...
		// flow.getDestination(),demandInSlots, path);
		pathBack = getbestPathBack(flow.getSource(), flow.getDestination(), demandInSlots, path);

		if (pathBack == null || pathBack.getLinks().length < 1 || pathBack.getLinks() == null
				|| pathBack.getSlotList() == null || path.getSlotList().isEmpty()) {
			/*
			 * if(pathBack == null){ for (int p : path.getLinks()){ System.out.print(p+" ");
			 * } }
			 */
...
//	Diz se dois vetores são disjuntos
	...
	public Path getShortestPath(MultiGraph multigraph, int src, int dst, int demand) {
		...
		Double nowCps = Double.MAX_VALUE;
			...
			for (int i = 0; i < multigraph.getNumEdges(); i++) {
				...
				if (nowpath == null || nowpath.size() < 2) {
					...
					if (nowpath.size() < path.size() || path.isEmpty()
							|| cps(j, nowpath.stream().mapToInt(k -> k).toArray()) < nowCps) {
						path = nowpath;
						nowSlot = i;
						nowCore = j;
						nowCps = cps(j, nowpath.stream().mapToInt(k -> k).toArray());
					}
				}
	...
	public Path getbestPathBack(int src, int dst, int demand, Path path) {
		...
		for (int i = 0; i < path.getLinks().length; i++) {
			multigraphb.removeEdge(pt.getLink(path.getLink(i)).getSource(),
					pt.getLink(path.getLink(i)).getDestination());
		}
	...
	public Path getbestPathBack(MultiGraph multigraph, int src, int dst, int demand, Path path) {
		Path backupPath;
		for (int i = 0; i < path.getLinks().length; i++) {
			multigraph.removeEdge(pt.getLink(path.getLink(i)).getSource(),
					pt.getLink(path.getLink(i)).getDestination());
			multigraph.removeEdge(pt.getLink(path.getLink(i)).getDestination(),
					pt.getLink(path.getLink(i)).getSource());
		}
