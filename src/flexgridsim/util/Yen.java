...
public final class Yen implements KSPAlgorithm {
    ...
    public int[][] ksp(WeightedGraph graph, int sourceLabel, int targetLabel, int K) {
    	List<Path> ksp = ksp(new Graph(graph), sourceLabel+"", targetLabel+"", K);
    	int kspMatrix[][] = new int[K][];
    	for (int i = 0; i < ksp.size(); i++) {
			kspMatrix[i] = ksp.get(i).toNodesArray();
		}
    	return kspMatrix;
    }
