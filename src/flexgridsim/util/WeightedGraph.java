...
public class WeightedGraph {
	...
	public WeightedGraph(WeightedGraph g) {
		...
		visited = new boolean[numNodes]; //Changed
		edgeRemoved = new boolean[numNodes][numNodes];
	...
	/**
	 * Removes a given edge from the graph by simply attributing zero to its
	 ...
	 * @param node the node
	 */
	public void markAsNotVisited(int node){
	 ...
	 * @param dst the dst
	 */
	public void markEdgeRemoved(int src, int dst){
		edgeRemoved[src][dst] = true;
	}
	...
	/**
	 * Restore edges removed.
	 */
	public void restoreEdgesRemoved(){
		for (int i = 0; i < numNodes; i++) {
			for (int j = 0; j < numNodes; j++) {
				edgeRemoved[i][j] = false;
			}
        ...
        public void restoreEdgeRemoved(int src, int dst) {
		edgeRemoved[src][dst] = false;
	}
